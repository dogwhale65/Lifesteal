...
public class PageManager {
    ...
    public void populateCurrentPage(Container inventory, List<PlayerCollector.Revivable> revivables,
        ...
        for (int i = startIndex; i < endIndex; i++) {
            ...
            inventory.setItem(contentSlot(i - startIndex), factory.createPlayerHead(entry.name(), entry.banned()));
        }
    ...
    /**
     * Maps a 0-based position on the current page to the chest slot that holds it. Heads occupy the
     * padded interior of the grid rather than filling the chest, so the position is not the slot.
     */
    private static int contentSlot(int position) {
        int row = Constants.GRID_FIRST_ROW + position / Constants.GRID_COLS;
        int col = Constants.GRID_FIRST_COL + position % Constants.GRID_COLS;
        return row * Constants.CHEST_COLUMNS + col;
    }
